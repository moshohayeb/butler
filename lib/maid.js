...
function completer(cline, commands) {
    ...
    line.parse();
    console.log("Error:", line.getError());
    return line.getCompleteList();
}

function processor(command) {
    var line = Line(_.trim(command), this._commands);
    line.parse();
    if (line.error()) {
      var emsg = '%%' + line.getErrorMsg() + '\r\n';
...
function _tabComplete() {
    ...
    var predicate   = function (element) { return element.help[0] === '!'; }
