...
function normalize(options) {
  ...
  opts = _.map(options, function (opt) {
    ...
    option.completions = support.resolveMatches(opt.match, opt.matchName, opt.matchHelp)
...
util.inherits(Option, Base)


Option.prototype.validate = function () {
  ...
  if (options.length === 1 && options[0].primary) {;}
  else if (options.length > 0 && min > this.optionCount) {
    this._errorToken = null
...
Option.prototype.parse = function () {
  var currentToken
  var currentOption
  var tokens
  var primaryOption
...
Option.prototype.complete = function () {
  ...
  if (shouldPrint) {
    ...
    primaryList = support.transform(primaryList)
