...
(function () {
  ...
  function parse() {
    ...
    primaryKey    = _.find(options, { primary: true });
    primaryKey    = primaryKey ? primaryKey.name : null;
    ...
    while (tokens.length !== 0 && tokens[0] !== '|') {
      ...
      if (!currentOption) {
        if (primaryExists && !this.primary) {
          primaryExists     = false;
          store[primaryKey] = currentKey;
          this.seenOptions.push(primaryKey)
          continue;
        }
