...
(function () {
  ...
  function checkValue(option) {
    ...
    console.log(_.isRegExp(possible.match));
    if (possible.length === 1
      && _.has(possible, 'name')
      && _.isRegExp(possible.name)) {
      if (!possible.test(value)) {
        this._errorMsg = util.format('value for option `%s` does not match accepted regex', key);
        return false;
      }
      return true;
    }
  ...
  function completes() {
    ...
    var shouldPrint = (this.completions.length || _.keys(this.context.store).length > 0) &&
      _.include([STATE.ON_MULTI_KEY, STATE.ON_SINGLE_KEY], this.currentState);

    if (shouldPrint) {
      this.completions.push({name: '<cr>', help: 'execute command', completeOn: false})
      if (pipeCommand)
