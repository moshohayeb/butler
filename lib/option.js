...
//   flexable: will accept any value is the options value
...
function normalize(options) {
  ...
  opts    = _.map(options, function (opt) {
    ...
    // same with multiple
    option.primary = primary ? false : ((!option.bool && !option.multiple && !option.group && opt.primary) || false)
...
function parse() {
  ...
  function processMultipleOptions() {
    ...
    while (tokens.length !== 0) {
      ...
      choiceCompletion.push({ name: element, help: '!' + fig.arrowLeft + ' selected' })
    }
  ...
  while (tokens.length !== 0 && tokens[0] !== PIPE_CHR) {
    ...
    var finder = function (element) {
      if (hasPO && currentKey === primaryOption.name) return false
    ...
    if (!currentOption) {
      if (hasPO && checkValue.call(this, primaryOption, currentKey)) {
  ...
  if (this.currentState !== STATE.ON_MULTI_KEY) { choiceCompletion = [] }
