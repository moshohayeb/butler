...
(function () {
  ...
  function normalize(options) {
    ...
    opts    = _.map(options, function (opt) {
      ...
      // prefill boolean options will false
      // in the store
      if (option.bool)
  ...
  function parse() {
    ...
    while (tokens.length !== 0 && tokens[0] !== '|') {
      ...
      // you can't specify the primary option, it is just entered without
      // its name
      if (primaryOption && primaryOption.name === currentOption.name) {
        this._errorMsg   = util.format('unknown option: %s', currentKey);
        this._errorToken = currentKey;
        return false;
      }
