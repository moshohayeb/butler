...
(function () {
  ...
  function checkValue(option) {
    ...
    if (_.isArray(possible) && possible.length === 1 && possible[0].name[0] === '<') {
      possible = possible[0];
    }
  ...
  function normalize(options) {
    ...
    opts = _.map(options, function (opt) {
      ...
      option.primary     = primary ? false : (opt.primary || false);
  ...
  function parse() {
    ...
    _.each(options, function (option) {
      if (_.has(store, option.name)) return; // option already inserted
      store[option.name] = option.default;      // whatever if undefined
    });

    primaryExists = _.any(options, {primary: true});
    ...
    // === post process === //
    // TODO: See if can better be done with lodash
    ...
    this.completions =
      this.currentState == STATE.ON_VALUE ?
        this.completions :
        cleanup.call(this, options).concat(choiceCompletion);

    return true;
  }

  function complete() {
    var primaryC     = this.primaryOption ? this.primaryOption.completions : null;
    var completeList = this.completions;
    var plist        = utility.transform(primaryC, true)
    ...
    if (shouldPrint) {
      ...
      if (this.primaryOption && !_.contains(this.seenOptions, this.primaryOption.name)) {
        _.each(plist, function (p) { p.help = '_' + p.help });
        completeList = completeList.concat(plist);
      }
    }
