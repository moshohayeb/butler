...
util.inherits(Butler, Readline.Interface)
...
function processor(cline) {
  ...
  var commands    = this._commands
  var config      = this._config
  var done        = this.prompt.bind(this)
  var line
...
function tab(show) {
  ...
  for (i = 0; i < multipleList.length; i++) {
    if (i === 0) {
      this.write('\r\n')
      this.write('   Remaining selections for option>'.bold)
      this.write('\r\n')
    }
...
Butler.prototype.write = function (chunk) {
  this._write(chunk)
}

Butler.prototype.start = function () {
  this._motd(console.log)
  this.write('\r\n')
