...
function run(done) {
  return this.parsers.command.run.call(this, done)
}

function Line(rl) {
  if (!(this instanceof Line)) { return new Line(rl) }

  var line

  // line should be a string
  try { line = rl.line.toString() } catch (e) { line = '' }

  line = _.trimLeft(line)
  ...
  this.context.tokens      = _.words(line, /[^ ]+/g)
  this.context.partialWord = _.last(line) !== ' ' ? this.context.tokens.pop() : null
  this.context.blueprint   = _.isArray(rl._commands) ? rl._commands : []
  this.context.config      = rl._config || {}
  this.rl                  = rl
