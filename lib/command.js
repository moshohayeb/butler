...
(function () {
  ...
  function validate() {
    var commandPath = _.trim(this.commandPath);
    // check that the command is valid
    if (!this.executable) {
      this._errorMsg = util.format('incomplete command `%s`', commandPath);
      return false;
    }

    // check that there is a run context for the command
    if (!_.isFunction(this.run)) {
      this._errorMsg = util.format('command `%s` has no runnable context', commandPath);
      return false;
    }
