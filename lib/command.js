...
(function () {
  ...
  function cleanUpCommands(command) {
    ...
    var cleanCommand  = {};
    var recursiveDeep = function (cmd) { return cleanUpCommands(cmd) };
    ...
    if (_.has(command, 'commands')) {
      // subcommand tree
      cleanCommand.commands = _.map(command.commands, recursiveDeep);
    } else {
      // final command
  ...
  function complete() {
    ...
    if (this.executable) {
      completions.push({ name: '<cr>', help: '', completeOn: false });
      if (this.context.pipeable)
        completions.push({ name: '|', help: 'pipe output to an external command', completeOn: true })
    }
