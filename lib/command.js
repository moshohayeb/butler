...
var fs      = require('fs')
var dcp     = require('duplex-child-process').spawn
var through = require('through2')
var shell   = require('shell-quote')

var Base = require('./base')

var noop = through.ctor(function (chunk, encoding, next) {
  next(null, chunk)
})
...
function run(done) {
  ...
  runnable = this.parsers.command.runnable
  ...
  if (pipe) {
    va       = shell.parse(pipe)
    pipeCmd  = va.shift()
    modifier = dcp(pipeCmd, va)
  } else {
    modifier = dcp('cat', ['-u', '-e', '-'])
  }

  pre.pipe(modifier).pipe(post).pipe(this.rl.output)
  post.on('end', function () { done() })
  runnable.call(this.rl, pre, this.context.store)
}
