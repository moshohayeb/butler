...
function run(terminal, done) {
  ...
  var display = function (output) {
    ...
    cp.execSync('more', {
      input:    output,
      stdio:    [null, terminal, 'ignore'],
      encoding: 'utf8'
    })
    done()
  }

  var processCommandOutput = function (output) {
    var pipe = self.context.pipe
    var processed

    if (!pipe) { return display(output) }

    try {
      processed = cp.execSync(pipe, {
        input:    output,
        stdio:    [null, null, 'ignore'],
        encoding: 'utf8'
      })
      return display(processed)
    } catch (error) { return done() /* do nothing */ }
  }

  var stream   = concat({ encoding: 'string' }, processCommandOutput)
  var runnable = this.runnable
  runnable.call(null, this.context.store, stream)
}
